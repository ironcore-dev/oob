...
APIS_APPLYCONFIGURATION='github.com/ironcore-dev/oob/api/v1alpha1'
...
go run k8s.io/code-generator/cmd/openapi-gen \
  ...
  --output-package "github.com/ironcore-dev/oob/openapi" \
  -O zz_generated.openapi \
  --report-filename "openapi/api_violations.report"

go run github.com/ironcore-dev/ironcore/models-schema --openapi-package "github.com/ironcore-dev/oob/openapi" --openapi-title "oob" > "$MODELSSCHEMA"
go run k8s.io/code-generator/cmd/applyconfiguration-gen \
  --output-base "$GOPATH/src" \
  --go-header-file hack/boilerplate.go.txt \
  --input-dirs "$APIS_APPLYCONFIGURATION" \
  --openapi-schema "$MODELSSCHEMA" \
  --output-package "github.com/ironcore-dev/oob/applyconfiguration"
